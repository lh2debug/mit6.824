...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	rf.mu.Lock()
	//fmt.Printf("Me:%d, Start command:%v\n",rf.me, command)
	defer rf.mu.Unlock()
	index := -1
	term := rf.currentTerm
...
func (rf *Raft) ReApply(index int) bool{
	
	fmt.Printf("ReApply Me:%d, log len:%d， commitIndex:%d\n", rf.me, len(rf.log), rf.commitIndex)
	if len(rf.log) == 0{
		return false
	}
	...
	baseIndex := rf.log[0].LogIndex
	if commitIndex > 1{
		fmt.Printf("Real Reply\n")
	}
	for i := index; i <= commitIndex; i++ {
