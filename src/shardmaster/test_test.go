...
func check_same_config(t *testing.T, c1 Config, c2 Config) {
	if c1.Num != c2.Num {
		fmt.Printf("c1 Num:%d, c2 Num:%d", c1.Num, c2.Num)
		t.Fatalf("Num wrong")
	}
	...
	if len(c1.Groups) != len(c2.Groups) {
		fmt.Printf("c1 length: %d, c2 length: %d \n", len(c1.Groups), len(c2.Groups))
...
func TestBasic(t *testing.T) {
	...
	for s := 0; s < nservers; s++ {
		...
		for i := 0; i < len(cfa); i++ {
			fmt.Printf("Query Test i:%d, cfa[i].Num:%d \n", i, cfa[i].Num)
	...
	//WeiHaocheng modify: 10 is impossible
	const npara = 5
	var cka [npara]*Clerk
	for i := 0; i < len(cka); i++ {
