...
func TestStaticShards(t *testing.T) {
	...
	ka := make([]string, n)
	va := make([]string, n)
	fmt.Printf("Before Put\n")
	for i := 0; i < n; i++ {
		ka[i] = strconv.Itoa(i) // ensure multiple shards
		va[i] = randstring(20)
		fmt.Printf("ck.config.num:%d\n", ck.config.Num)
		ck.Put(ka[i], va[i])
	}
	fmt.Printf("After Put\n")
	for i := 0; i < n; i++ {
	...
	// Get()s don't succeed.
	fmt.Printf("Before ShutdownGroup\n")
	cfg.ShutdownGroup(1)
	cfg.checklogs() // forbid snapshots
	fmt.Printf("After ShutdownGroup 1\n")
	...
	fmt.Printf("After ShutdownGroup 2\n")
	...
	fmt.Printf("Before StartGroup1, ck.ReqId:%d\n", ck.ReqID)
	cfg.StartGroup(1)
	
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
	}
	fmt.Printf("After StartGroup1\n")
...
func TestSnapshot(t *testing.T) {
	...
	fmt.Printf("Before Shutdown!!\n")
	
	cfg.ShutdownGroup(0)
	cfg.ShutdownGroup(1)
	cfg.ShutdownGroup(2)

	fmt.Printf("After Shutdown, before Start!!\n")
